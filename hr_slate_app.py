...
def *norm(df):
return df.rename(columns={c: c.lower().replace(” “,”*”).replace(”%”,”*pct”)
.replace(”/”,”*per*”).replace(”-”,”*”) for c in df.columns})
def _std_id(df):
...
@st.cache_data(ttl=3600, show_spinner=False)
def get_ab_log(batter_id, season, n_abs=15, pitcher_hand=None, pitch_types=None):
“””
Fetch last N at-bats (plate appearances with a result) for a batter from Savant.
Filters by pitcher_hand (R/L) and pitch_types (list of codes e.g. [‘FF’,‘SL’]).
Returns list of dicts, one per AB, sorted most-recent first.
“””
pid = str(batter_id).replace(”.0”,””)
for s in (season, SEASON_FALLBACK):
df = _get_batter_raw_df(pid, s)
if df is None: continue

```
    # Keep only pitches with a real AB result
    if "events" not in df.columns: continue
    df = df.copy()
    ...
    if pitch_types and "pitch_type" in df.columns:
        df = df[df["pitch_type"].str.upper().isin([p.upper() for p in pitch_types])]

    # Keep only rows that are the "last pitch of an AB" (has an event result)
    ab_events_mask = df["events"].notna() & df["events"].isin(
        AB_EVENTS | {"walk","hit_by_pitch","sac_fly","sac_bunt","catcher_interf"})
    ab_df = df[ab_events_mask].copy()
    if ab_df.empty: continue

    ab_df = ab_df.sort_values("game_date", ascending=False).head(n_abs)
    ...
    for _, row in ab_df.iterrows():
        ...
        # Barrel
        is_brl = _is_barrel(
            float(ev) if ev is not None and not (isinstance(ev,float) and np.isnan(ev)) else None,
            float(la) if la is not None and not (isinstance(la,float) and np.isnan(la)) else None
        ...
        if dist is not None and not np.isnan(float(dist)):
            ...
            if d >= 430: hr_parks = "30/30"
            elif d >= 410: hr_parks = "25/30"
            elif d >= 390: hr_parks = "18/30"
            elif d >= 370: hr_parks = "10/30"
            elif d >= 350: hr_parks = "4/30"
            elif d >= 330: hr_parks = "1/30"
            else: hr_parks = "0/30"
        elif result.lower() == "home run":
            hr_parks = "✓ HR"
        ...
        rows.append({
            ...
            "pitch_type": pt_nm,
            "pt_code": pt,
            "ev": round(float(ev),2) if ev is not None and not np.isnan(float(ev)) else None,
            "la": round(float(la),2) if la is not None and not np.isnan(float(la)) else None,
            "distance": round(float(dist),1) if dist is not None and not np.isnan(float(dist)) else None,
            "bat_speed": round(float(bat_spd),1) if bat_spd is not None and not np.isnan(float(bat_spd)) else None,
            "pitch_velo": round(float(pitch_velo),1) if pitch_velo is not None and not np.isnan(float(pitch_velo)) else None,
            "result": result,
            "trajectory": traj,
            "hr_parks": hr_parks,
            "is_barrel": is_brl,
            "is_hr": result.lower() == "home run",
            "season": s,
        })
    if rows: return rows
return []
...
if fg is not None and not fg.empty:
fg = *norm(fg)
fg = *scale_pct(fg,[“k_pct”,“bb_pct”,“fb_pct”,“gb_pct”,“ld_pct”,
...
fg = fg.rename(columns={c:“p*”+c for c in fg.columns if c not in skip and not c.startswith(“p*”)})
fg = _std_id(fg)
else: fg=None
if sc is not None and fg is not None and “player_id” in fg.columns:
sc[“player_id”]=sc[“player_id”].astype(str)
...
# _score() linearly maps value â†’ 0-100 within [lo, hi]
...
metrics = [
# col            lo    hi   hib   wt
(“barrel_pct”,    3,   18,  True, 28),
(“hard_hit_pct”, 30,   55,  True, 22),
(“avg_ev”,       86,   95,  True, 18),
(“avg_la”,        8,   18,  True,  8),
(“sweet_spot_pct”,25,  42,  True,  8),
(“fb_pct”,       22,   45,  True,  7),
(“pull_pct”,     28,   50,  True,  5),
(“k_pct”,         6,   30, False,  4),
]
total_max_w = sum(w for *_,w in metrics)
total_w = 0; total_s = 0
...
“””
metrics = [
(“barrel_pct”,    28), (“hard_hit_pct”, 22), (“avg_ev”,       18),
(“avg_la”,         8), (“sweet_spot_pct”, 8), (“fb_pct”,        7),
(“pull_pct”,       5), (“k_pct”,          4),
]
total_w = sum(w for _,w in metrics)
found_w = sum(w for col,w in metrics if _fv(h_row, col) is not None)
return round(found_w / total_w, 2), found_w, total_w
...
```
Two modes:
- If p_row has real data: score only from available metrics (skip missing ones),
  requires at least 35 weight-points to return a score.
- If p_row is empty/None (TBD pitcher, no data found): substitute full league
  averages and return ~50 so the HR score still computes.
"""
MLB_AVG = {
    "p_brl_pct":      6.5,
    "p_hr_per_fb":   12.0,
...
has_real_data = p_row and any(_fv(p_row, col) is not None for col,*_ in metrics)
...
return round(total_s / total_w) if total_w >= 35 else None
```
...
“””
metrics = [
(“p_brl_pct”,30), (“p_hr_per_fb”,25), (“p_hard_hit_pct”,20),
(“p_k_pct”,15),   (“p_fb_pct”,7),     (“p_avg_ev”,3),
]
total_w = sum(w for _,w in metrics)
found_w = sum(w for col,w in metrics if p_row and _fv(p_row, col) is not None)
return round(found_w / total_w, 2), found_w, total_w
...
wc4.metric(“ðŸŸï¸ Park”,f”{pf:.2f}Ã– ({home})”)
...
_sc_bat_ok  = “barrel_pct”    in _cols_h
_fg_bat_ok  = “pull_pct”      in _cols_h
_sc_pit_ok  = “p_brl_pct”     in _cols_p
_fg_pit_ok  = any(c in _cols_p for c in (“p_k_pct”,“era”,“fip”))

_status_parts = [
f”{‘✅’ if _sc_bat_ok else ‘❌’} Batter Statcast”,
f”{‘✅’ if _fg_bat_ok else ‘❌’} Batter FanGraphs”,
f”{‘✅’ if _sc_pit_ok else ‘❌’} Pitcher Statcast”,
f”{‘✅’ if _fg_pit_ok else ‘❌’} Pitcher FanGraphs”,
]
if not all([_sc_bat_ok, _fg_bat_ok, _sc_pit_ok, _fg_pit_ok]):
st.warning(
f”âš ï¸ Some data sources failed to load – Hitter/Pitcher scores may be incomplete. “
...
f”league-average fallbacks for missing data.”,
icon=“âš ï¸”
)
else:
...
def _conf_cell(conf):
“”“Render a confidence score as a compact colored badge.”””
...
bg, tc = “#fee2e2”, “#991b1b”
return (f”<span style='display:inline-block;padding:2px 7px;border-radius:5px;"
f"font-size:.73rem;font-weight:700;background:{bg};color:{tc}'>{pct}%</span>”)
...
pr=calc_pitcher_hr_risk(p_row)
p_conf,_,_ = calc_pitcher_confidence(p_row)
...
for col,(k,lbl) in zip(sc_cols,sort_opts.items()):
    ...
    icon="â-¼ " if active and not st.session_state.sort_asc else ("â-² " if active else "")
...
for i,h in enumerate(hitters):
    ...
    rows.append({
        ...
        "h_conf":h_conf,"p_conf":p_conf,  # confidence scores
...
hdr=(f"<div style='overflow-x:auto'><table class='slate-tbl'><thead>"
     ...
     f"<th rowspan='2' title='Data confidence'>H CONF</th>"
...
if sel_hid and st.session_state.selected_team_key==tab_key:
    ...
    if det:
        ...
        with r2b:
            ...
            st.caption(f"Source: Savant {SEASON}â†'{SEASON_FALLBACK} fallback")
```
...
tab_away,tab_home,tab_pitchers,tab_scores,tab_ab=st.tabs([
...
with tab_pitchers:
st.markdown(”### ⚔️ Pitcher Score Breakdown”)
...
with tab_scores:
st.markdown(”### 📊 Score Breakdown & Methodology”)
...
with info_c1:
    if True:  # was expander
        ...
        # Build full hitter list for inline selector
        all_cached_rows = st.session_state.get('rows_away',[]) + st.session_state.get('rows_home',[])

        sel_hitter_row = {}
        sel_hitter_name = "No hitter selected"

        if all_cached_rows:
            hitter_options = [(str(r.get("id","")), f"{r.get('name','')} ({r.get('team','')})")
                              for r in all_cached_rows if r.get("name")]

            # Find current selection index
            current_id = str(sel_id) if sel_id else ""
            current_idx = next((i for i,(rid,_) in enumerate(hitter_options) if rid==current_id), 0)

            sd_col1, sd_col2 = st.columns([3,1])
            with sd_col1:
                picked_label = st.selectbox(
                    "Select hitter to analyse:",
                    [lbl for _,lbl in hitter_options],
                    index=current_idx,
                    key="sb_hitter_pick",
                    label_visibility="collapsed"
                )
            with sd_col2:
                st.markdown("<div style='font-size:.72rem;color:#64748b;padding-top:8px'>â† pick any hitter</div>",
                            unsafe_allow_html=True)

            picked_id = next((rid for rid,lbl in hitter_options if lbl==picked_label), None)
            if picked_id:
                picked_row = next((r for r in all_cached_rows if str(r.get("id",""))==picked_id), None)
                if picked_row:
                    sel_hitter_row = picked_row.get("_h_row", {})
                    sel_hitter_name = picked_row.get("name","")
        else:
            st.info("⬆ï¸ Load a game and open a team tab first -- hitter stats will appear here once loaded.")
        ...
        if sel_hitter_name != "No hitter selected":
            ...
            if not sel_hitter_row:
                st.warning(
                    "âš ï¸ No Statcast/FanGraphs data for this hitter -- pybaseball may not be "
        ...
        PITCHER_METRICS = [
            # (label, col, lo, hi, hib_for_score, dec, suf, weight)
            # hib_for_score=True means higher value = more HR risk = higher score
            # val_chip uses NOT hib so color shows pitcher safety: low barrel% = green
        ...
        HR_COMPONENTS = [
            ...
            ("ðŸŸï¸ Park Factor",    round(pf_score_val), 0.05, f"{home} · {pf_val:.2f}Ã-- factor"),
...
<tbody>
<tr><td><b>Barrel %</b></td><td><span class='src-tag src-sc'>Statcast</span></td><td class='range-lbl'>4% â†' 15%</td><td><span class='dir-g'>â†' Higher</span></td><td>Hitter Score (28wt), HR Score</td></tr>
<tr><td><b>Hard Hit %</b></td><td><span class='src-tag src-sc'>Statcast</span></td><td class='range-lbl'>32% â†' 52%</td><td><span class='dir-g'>â†' Higher</span></td><td>Hitter Score (22wt)</td></tr>
<tr><td><b>Exit Velocity</b></td><td><span class='src-tag src-sc'>Statcast</span></td><td class='range-lbl'>86 â†' 94 mph</td><td><span class='dir-g'>â†' Higher</span></td><td>Hitter Score (18wt)</td></tr>
<tr><td><b>Launch Angle</b></td><td><span class='src-tag src-sc'>Statcast</span></td><td class='range-lbl'>8° â†' 18°</td><td><span class='dir-g'>â†' Higher</span> (in range)</td><td>Hitter Score (8wt)</td></tr>
<tr><td><b>Sweet Spot %</b></td><td><span class='src-tag src-sc'>Statcast</span></td><td class='range-lbl'>25% â†' 42%</td><td><span class='dir-g'>â†' Higher</span></td><td>Hitter Score (8wt)</td></tr>
<tr><td><b>Fly Ball %</b></td><td><span class='src-tag src-fg'>FanGraphs</span></td><td class='range-lbl'>22% â†' 42%</td><td><span class='dir-g'>â†' Higher</span></td><td>Hitter Score (7wt)</td></tr>
<tr><td><b>Pull %</b></td><td><span class='src-tag src-fg'>FanGraphs</span></td><td class='range-lbl'>28% â†' 48%</td><td><span class='dir-g'>â†' Higher</span></td><td>Hitter Score (5wt)</td></tr>
<tr><td><b>K %</b></td><td><span class='src-tag src-fg'>FanGraphs</span></td><td class='range-lbl'>8% â†' 28%</td><td><span class='dir-r'>â†" Lower</span></td><td>Hitter Score (4wt)</td></tr>
<tr><td><b>BB %</b></td><td><span class='src-tag src-fg'>FanGraphs</span></td><td class='range-lbl'>5% â†' 14%</td><td><span class='dir-g'>â†' Higher</span></td><td>Display only</td></tr>
<tr><td><b>SwStr %</b></td><td><span class='src-tag src-fg'>FanGraphs</span></td><td class='range-lbl'>5% â†' 14%</td><td><span class='dir-r'>â†" Lower</span></td><td>Display + Last 14 Days</td></tr>
<tr><td><b>AVG / OBP / SLG</b></td><td><span class='src-tag src-fg'>FanGraphs</span></td><td class='range-lbl'>Standard</td><td><span class='dir-g'>â†' Higher</span></td><td>Display only</td></tr>
<tr><td><b>wOBA / xwOBA</b></td><td><span class='src-tag src-sc'>Statcast</span></td><td class='range-lbl'>.290 â†' .390</td><td><span class='dir-g'>â†' Higher</span></td><td>Pitch mix score, platoon score</td></tr>
...
<tbody>
<tr><td><b>Barrel % Allowed</b></td><td><span class='src-tag src-sc'>Statcast</span></td><td class='range-lbl'>2% â†' 14%</td><td><span class='dir-r'>â†' High</span></td><td>30 -- strongest predictor</td></tr>
<tr><td><b>HR / FB %</b></td><td><span class='src-tag src-fg'>FanGraphs</span></td><td class='range-lbl'>5% â†' 20%</td><td><span class='dir-r'>â†' High</span></td><td>25</td></tr>
<tr><td><b>Hard Hit % Allowed</b></td><td><span class='src-tag src-sc'>Statcast</span></td><td class='range-lbl'>28% â†' 50%</td><td><span class='dir-r'>â†' High</span></td><td>20</td></tr>
...
.ab-tbl th{background:#1e293b;color:#94a3b8;padding:7px 10px;text-align:center;
  font-size:.65rem;text-transform:uppercase;letter-spacing:.05em;white-space:nowrap;
  border-bottom:2px solid #334155}
.ab-tbl th:first-child{text-align:left}
.ab-tbl td{padding:6px 10px;text-align:center;border-bottom:1px solid #1e293b;
  white-space:nowrap;font-family:'Courier New',monospace;font-size:.8rem}
.ab-tbl td:first-child{text-align:left;font-family:inherit}
.ab-tbl tr:hover td{background:#f8fafc}
.ab-hr td{background:#fef9c3 !important}
.ab-brl td{background:#dcfce7 !important}
.ab-out td{background:#ffffff}
.ab-ev-hot{color:#16a34a;font-weight:700}
.ab-ev-warm{color:#ca8a04;font-weight:600}
.ab-ev-cold{color:#dc2626}
.ab-tag{display:inline-block;padding:1px 7px;border-radius:4px;font-size:.72rem;font-weight:700}
.tag-hr{background:#fef08a;color:#854d0e}
.tag-brl{background:#bbf7d0;color:#14532d}
.tag-hit{background:#dbeafe;color:#1e40af}
.tag-out{background:#f1f5f9;color:#64748b}
</style>""", unsafe_allow_html=True)

    # ── Controls ─────────────────────────────────────────────────────────
    ab_c1, ab_c2, ab_c3, ab_c5 = st.columns([2, 1, 1, 1])

    # Collect all hitters from today's slate, tagged with their opposing pitcher arsenal + hand
    all_slate_hitters = []
    for team_key in ["away","home"]:
        for r in st.session_state.get(f"rows_{team_key}", []):
            opp_arsenal = away_arsenal if team_key == "home" else home_arsenal
            opp_hand    = home_sp_hand if team_key == "away"  else away_sp_hand
            all_slate_hitters.append((r["id"], r["name"], r["team"], team_key, opp_arsenal, opp_hand))
    all_slate_hitters = list({x[0]:x for x in all_slate_hitters}.values())

    with ab_c1:
        if all_slate_hitters:
            h_options = [f"{name} ({team})" for _,name,team,_,_,_ in all_slate_hitters]
            prev_pick = st.session_state.get("ab_batter_prev", "")
            ab_pick = st.selectbox("Batter", h_options, key="ab_batter")
            ab_idx = h_options.index(ab_pick)
            ab_hitter_id   = all_slate_hitters[ab_idx][0]
            ab_hitter_name = all_slate_hitters[ab_idx][1]
            ab_opp_arsenal = all_slate_hitters[ab_idx][4]
            ab_opp_hand    = all_slate_hitters[ab_idx][5]  # "R" or "L"

            # When batter changes, auto-set pitch types AND pitcher arm
            if ab_pick != prev_pick:
                st.session_state["ab_batter_prev"] = ab_pick
                # Auto-set pitcher arm
                st.session_state["ab_hand"] = "RHP" if ab_opp_hand == "R" else "LHP"
                # Auto-set pitch types from arsenal
                if ab_opp_arsenal:
                    auto_pts = [PITCH_NAMES[pt] for pt in
                                sorted(ab_opp_arsenal, key=lambda x: -ab_opp_arsenal[x])
                                if pt in PITCH_NAMES]
                    st.session_state["ab_pitches"] = auto_pts
                else:
                    st.session_state["ab_pitches"] = ["All Pitches"]
                st.rerun()
        else:
            st.info("Load a game above first")
            st.stop()

    with ab_c2:
        ab_count = st.selectbox("Last N ABs", [10, 15, 20, 30, "Season"], key="ab_count")
        n_abs = 500 if ab_count == "Season" else int(ab_count)

    with ab_c3:
        ab_hand = st.selectbox("Pitcher Arm", ["Both", "RHP", "LHP"],
                               index=["Both","RHP","LHP"].index(
                                   st.session_state.get("ab_hand","Both")),
                               key="ab_hand")
        pitcher_hand_filter = None if ab_hand == "Both" else ab_hand[0]
    ...
    # Build arsenal chip display for context
    if ab_opp_arsenal:
        opp_sp_name_ab = home_sp_name if all_slate_hitters[ab_idx][3]=="away" else away_sp_name
        arsenal_hint = " · ".join(
    ...
    if "All Pitches" in ab_pitches or not ab_pitches:
        pitch_filter = None
    else:
        pitch_filter = [all_pitch_names[p] for p in ab_pitches if p in all_pitch_names]

    # ── Fetch + summary strip ─────────────────────────────────────────────
    with st.spinner(f"Loading {ab_hitter_name}'s recent ABs…"):
        ab_rows = get_ab_log(ab_hitter_id, SEASON, n_abs, pitcher_hand_filter, pitch_filter)
    ...
    if not ab_rows:
        ...
        valid_la  = [r["la"]   for r in ab_rows if r["la"]   is not None]
        n_hrs     = sum(1 for r in ab_rows if r["is_hr"])
        n_brl     = sum(1 for r in ab_rows if r["is_barrel"])
        n_bip     = len(valid_ev)
        avg_ev    = round(sum(valid_ev)/len(valid_ev),1)  if valid_ev else None
        avg_la    = round(sum(valid_la)/len(valid_la),1)  if valid_la else None
        brl_pct   = round(n_brl/n_bip*100,1)             if n_bip>0  else None
        hard_pct  = round(sum(1 for e in valid_ev if e>=95)/n_bip*100,1) if n_bip>0 else None
        yr_note   = f" ({ab_rows[0]['season']})" if ab_rows and ab_rows[0].get("season")==SEASON_FALLBACK else ""

        lbl = f"Last {len(ab_rows)} ABs{yr_note}" if ab_count != "Season" else f"2025 Season{yr_note}"
        st.markdown(
            ...
            f"<div style='font-size:.65rem;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em'>{lbl}</div>"
            f"<div style='font-size:1rem;font-weight:700;color:#f1f5f9'>{len(ab_rows)} ABs</div></div>"
            + "".join(
                f"<div style='background:#1e293b;border-radius:8px;padding:8px 14px;text-align:center'>"
        ...
        # ── AB Table ──────────────────────────────────────────────────────
        hdr_html = """<div style='overflow-x:auto'>
<table class='ab-tbl'><thead><tr>
...
<th>HR PARKS</th>
        ...
        for r in ab_rows:
            ...
            if r["is_hr"]:
                tag = "<span class='ab-tag tag-hr'>💥 HR</span>"
            elif res_lower in ("single","double","triple"):
                tag = f"<span class='ab-tag tag-hit'>{r['result']}</span>"
            elif "strikeout" in res_lower:
                tag = f"<span class='ab-tag tag-out'>K</span>"
            else:
                tag = f"<span class='ab-tag tag-out'>{r['result']}</span>"

            # Barrel badge
            brl_badge = " 🟢" if r["is_barrel"] else ""
            ...
            if hr_pk not in ("--","0/30","✓ HR"):
                ...
                try:
                    pk_n = int(pk_parts[0])
                    pk_col = "#16a34a" if pk_n >= 25 else ("#ca8a04" if pk_n >= 10 else "#64748b")
                except: pk_col = "#64748b"
                hr_pk_str = f"<span style='color:{pk_col};font-weight:600'>{hr_pk}</span>"
            elif hr_pk == "✓ HR":
                hr_pk_str = "<span style='color:#fbbf24;font-weight:700'>✓ HR</span>"
            else:
                hr_pk_str = f"<span style='color:#94a3b8'>{hr_pk}</span>"

            pt_color = PITCH_COLORS.get(r["pt_code"], "#94a3b8")

            rows_html += (
                f"<tr class='{row_cls}'>"
                f"<td>{r['date']}</td>"
                f"<td style='font-family:inherit'>{r['pitcher']}</td>"
                f"<td>{r['arm']}</td>"
        ...
        st.caption("🟡 = Home Run · 🟢 = Barrel · EV green ≥95 mph · LA green = 8-32° (ideal HR range) · HR Parks = estimated parks where ball would leave yard")
```
